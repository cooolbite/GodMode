...
> **For agentic workers:** REQUIRED SUB-SKILL: Use godmode:executing-plans to implement this plan task-by-task.

**Goal:** Add full GodMode support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/GodMode-codex` (lines 40-74)
...
- Reference: `.codex/GodMode-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'GodMode' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/GodMode-codex` (lines 212-280)
 ...
 * (personal skills override GodMode skills).
 *
 * @param {string} skillName - Name like "GodMode:brainstorming" or "my-skill"
 * @param {string} GodModeDir - Path to GodMode skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, GodModeDir, personalDir) {
    // Strip GodMode: prefix if present
    const forceGodMode = skillName.startsWith('GodMode:');
    const actualSkillName = forceGodMode ? skillName.replace(/^GodMode:/, '') : skillName;

    // Try personal skills first (unless explicitly GodMode:)
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/GodMode-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/GodMode-codex` (add import at top)
...
Run: `node -c .codex/GodMode-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/GodMode-codex
...
**Files:**
- Modify: `.codex/GodMode-codex` (lines 40-74)
...
Run: `.codex/GodMode-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/GodMode-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/GodMode-codex` (lines 16-38, approximately)
...
Run: `.codex/GodMode-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/GodMode.js`
...
const GodModeSkillsDir = path.join(homeDir, '.config/opencode/GodMode/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const GodModePlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/GodMode.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/GodMode.js
...
**Files:**
- Modify: `.opencode/plugin/GodMode.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "GodMode:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > GodMode)
...
Run: `node -c .opencode/plugin/GodMode.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the GodMode and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const GodModeSkills = skillsCore.findSkillsInDir(
            GodModeSkillsDir,
            'GodMode',
            3
          );
          ...
          const allSkills = [...personalSkills, ...GodModeSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install GodMode skills to ~/.config/opencode/GodMode/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'GodMode:';
...
```javascript
    'session.started': async () => {
      // Read godmode-core skill content
      const usingGodModePath = skillsCore.resolveSkillPath(
        'godmode-core',
        GodModeSkillsDir,
        personalSkillsDir
      );

      let usingGodModeContent = '';
      if (usingGodModePath) {
        const fullContent = fs.readFileSync(usingGodModePath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingGodModeContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- GodMode skills: \`GodMode:skill-name\` (from ~/.config/opencode/GodMode/skills/)
- Personal skills: \`skill-name\` (from ~/.config/opencode/skills/)
- Personal skills override GodMode skills when names match
`;

      // Check for updates (non-blocking)
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/GodMode')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/GodMode && git pull` to update GodMode.' :
        '';
...
**Below is the full content of your 'GodMode:godmode-core' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
### 1. Install GodMode Skills

```bash
# Clone GodMode skills to OpenCode config directory
mkdir -p ~/.config/opencode/GodMode
git clone https://github.com/obra/GodMode.git ~/.config/opencode/GodMode
```

### 2. Install the Plugin

The plugin is included in the GodMode repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/GodMode/.opencode/plugin/GodMode.js`
...
ln -s ~/.config/opencode/GodMode/.opencode/plugin/GodMode.js .opencode/plugin/GodMode.js
```
...
```
use use_skill tool with skill_name: "GodMode:brainstorming"
```
...
Personal skills override GodMode skills with the same name.
...
1. Check plugin file exists: `ls ~/.config/opencode/GodMode/.opencode/plugin/GodMode.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/GodMode/skills`
...
- Report issues: https://github.com/obra/GodMode/issues
- Documentation: https://github.com/obra/GodMode
```
...
GodMode works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/GodMode-codex use-skill GodMode:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/GodMode-codex bootstrap | head -30`
...
ls -l .opencode/plugin/GodMode.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/GodMode.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/GodMode-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/GodMode-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/GodMode.js` created with tools and hooks
