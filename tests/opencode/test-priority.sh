...
mkdir -p "$GODMODE_SKILLS_DIR/priority-test"
cat > "$GODMODE_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
...
if [ -f "$GODMODE_SKILLS_DIR/priority-test/SKILL.md" ]; then
...
# Test 2: Document personal vs bundled godmode priority
echo ""
echo "Test 2: Documenting personal vs GodMode priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled GodMode version instead of personal version"

# Test 3: Document project vs bundled godmode priority
echo ""
echo "Test 3: Documenting project vs personal/godmode priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled GodMode version instead of project version"

# Test 4: Test a non-colliding bundled godmode skill is still available
echo ""
echo "Test 4: Testing non-colliding godmode skill remains available..."

mkdir -p "$GODMODE_SKILLS_DIR/godmode-only-test"
cat > "$GODMODE_SKILLS_DIR/godmode-only-test/SKILL.md" <<'EOF'
---
name: godmode-only-test
description: godmode-only priority test skill
---
# GodMode Only Test Skill

PRIORITY_MARKER_GODMODE_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"godmode-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_GODMODE_ONLY_VERSION" "Non-colliding godmode skill is still registered"
