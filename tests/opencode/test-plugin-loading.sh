...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/godmode.js"
...
skill_count=$(find "$GODMODE_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $GODMODE_SKILLS_DIR"
    exit 1
fi

# Test 3: Check godmode-core skill exists (critical for bootstrap)
echo "Test 3: Checking godmode-core skill (required for bootstrap)..."
if [ -f "$GODMODE_SKILLS_DIR/godmode-core/SKILL.md" ]; then
    echo "  [PASS] godmode-core skill exists"
else
    echo "  [FAIL] godmode-core skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$GodMode_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/godmode/' "$GodMode_PLUGIN_FILE"; then
