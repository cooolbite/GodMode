...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$GodMode_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$GODMODE_SKILLS_DIR/godmode-core/SKILL.md" "$TEST_HOME/godmode-core.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$GodMode_PLUGIN_FILE" missing
}
