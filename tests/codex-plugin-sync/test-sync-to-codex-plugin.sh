...
EOF
    ...
    cat > "$repo/assets/godmode-small.svg" <<'EOF'
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/godmode/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/godmode/.fixture-keep"
    cat > "$repo/plugins/godmode/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/godmode/.fixture-keep
    git -C "$repo" add plugins/godmode/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/godmode/skills/example/agents"

    cat > "$repo/plugins/godmode/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/godmode/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/godmode/.codex-plugin" \
        "$repo/plugins/godmode/.private-journal" \
        "$repo/plugins/godmode/assets" \
        "$repo/plugins/godmode/skills/example/agents" \
        "$repo/plugins/godmode/skills/example"

    cat > "$repo/plugins/godmode/.codex-plugin/plugin.json" <<EOF
{
  "name": "godmode",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/godmode/assets/godmode-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/godmode/assets/app-icon.png"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/godmode/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/godmode/.codex-plugin/plugin.json \
        plugins/godmode/assets/app-icon.png \
        plugins/godmode/assets/godmode-small.svg \
        plugins/godmode/skills/example/agents/openai.yaml \
        plugins/godmode/skills/example/SKILL.md \
        plugins/godmode/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/godmode/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/godmode/.fixture-keep"
    printf 'stale ignored leak\n' > "$repo/plugins/godmode/.private-journal/leak.txt"
    git -C "$repo" add plugins/godmode/.fixture-keep
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/godmode/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/godmode/skills/example/agents/openai.yaml"
    ...
    assert_contains "$preview_section" "assets/godmode-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (GodMode-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/godmode-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/godmode/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/godmode" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/godmode-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/godmode'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/godmode-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/godmode-*" "Clean no-op local apply does not create sync branch in destination checkout"
